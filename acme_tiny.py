...
def setup_dns(domain, txt_challenge, hosted_zone_id, aws_access_key_id, aws_secret_access_key):
    session = boto3.Session()
    client = session.client(
        "route53", aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
...
def delete_dns(domain, txt_challenge, hosted_zone_id, aws_access_key_id, aws_secret_access_key):
    session = boto3.Session()
    client = session.client(
        "route53", aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
    resp = client.change_resource_record_sets(
...
def get_crt(account_key, csr, aws_hosted_zone, aws_access_key_id, aws_secret_access_key, log=LOGGER, CA=DEFAULT_CA, disable_check=False, directory_url=DEFAULT_DIRECTORY_URL, contact=None):
    ...
    for auth_url in order['authorizations']:
        ...
        setup_dns(domain, keydigest64, aws_hosted_zone, aws_access_key_id, aws_secret_access_key)

        # say the challenge is done
        _send_signed_request(challenge['url'], {}, "Error submitting challenges: {0}".format(domain))
        authorization = _poll_until_not(auth_url, ["pending"], "Error checking challenge status for {0}".format(domain))
        if authorization['status'] != "valid":
            delete_dns(domain, keydigest64, aws_hosted_zone, aws_access_key_id, aws_secret_access_key)
            raise ValueError("Challenge did not pass for {0}: {1}".format(domain, authorization))
        log.info("{0} verified!".format(domain))
        records_to_clean.append((domain, keydigest64))
    ...
    for domain, keydigest64 in records_to_clean:
        delete_dns(domain, keydigest64, aws_hosted_zone, aws_access_key_id, aws_secret_access_key)
...
def main(argv=None):
    ...
    parser.add_argument("--aws-hosted-zone", required=True, help="AWS hosted zone id")
    ...
    signed_crt = get_crt(args.account_key, args.csr, args.aws_hosted_zone, args.aws_access_key_id, args.aws_secret_access_key, log=LOGGER, CA=args.ca, disable_check=args.disable_check, directory_url=args.directory_url, contact=args.contact)
